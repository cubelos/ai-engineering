...
def test_clean_description_passes() -> None:
    """Benign project text passes moderation, injection, and PII checks."""
    check_input(CLEAN_DESCRIPTION, openai_client=_fake_moderation(flagged=False))


def test_moderation_flag_blocks() -> None:
    """OpenAI moderation flagged content raises InputGuardrailViolation with reason moderation."""
...
def test_moderation_network_failure_fails_open() -> None:
    """If the moderation API errors, the check is skipped and other layers still run."""
    class Boom:
        class moderations:
            @staticmethod
...
def test_prompt_injection_pattern_blocks(injection: str) -> None:
    """Known prompt-injection phrases are blocked with reason prompt_injection."""
...
def test_pii_email_blocks() -> None:
    """An email address in the description triggers the PII guardrail."""
...
def test_pii_iban_blocks() -> None:
    """An IBAN in the description triggers the PII guardrail."""
...
def test_pii_phone_blocks() -> None:
    """A phone number in the description triggers the PII guardrail."""
...
def test_openai_client_optional() -> None:
    """When openai_client is None, moderation is skipped and regex layers still apply."""
...
def test_order_is_moderation_then_injection_then_pii() -> None:
    """When multiple layers would fire, moderation wins because it runs first."""
